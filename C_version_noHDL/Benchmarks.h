...
void saveIntArrayToFile(const char* filename, const unsigned char* array, int length);
...
void runMemoryBenchmark(int numberOfTimesToReplaceR,int * hit_counter,int *miss_counter) {
	...
	if (printBenchmarkResult) {
		...
		saveIntArrayToFile("out.txt", setAccessedByOrderArray, numberOfSetsAccessed);
		printf("\n--------------------------------------------------------------- \n");
...
//setup function -should be invoked before starting the AES_benchmark 
void putSboxInMemArea() {
...
void saveIntArrayToFile(const char* filename, const unsigned char* array, int length) {
	...
	if (file != 0) {
		for (int i = 0; i < length; i++) {
			fprintf(file, "%d\n", array[i]); // Write each number followed by a newline
		}
